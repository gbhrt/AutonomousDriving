...
def generate_launch_description():
    ...
    track_traj_file_name = get_share_file('file_trajectory_planner', 'param/trajectories/full_trajectory-0.txt')
    traj_file_name_ego1 = get_share_file('file_trajectory_planner', 'param/trajectories/full_trajectory_TUM.txt')
    ...
    minimal_simulator = Node(
        ...
        parameters=[os.path.join(minimal_simulator_pkg_prefix, "param/vehicles/option2/ego_vehicle.param.yaml")],
        remappings=[
    ...
    vehicle_num = 3 #int(launch.substitutions.LaunchConfiguration('vehicle_num'))
    ...
    for i in range(1,vehicle_num+1):
        vehicle_nodes.append(
            Node(
                ...
                name="minimal_simulator_node",
                output="screen",
                parameters=[os.path.join(minimal_simulator_pkg_prefix, "param/vehicles/option2/vehicle"+str(i)+".param.yaml")
                ],
                remappings=[
    ...
    file_trajectory_planner_nodes = []
    for i in range(1,vehicle_num+1):
        traj_file_name = get_share_file('file_trajectory_planner', 'param/trajectories/option2/traj'+str(i)+'.txt')
